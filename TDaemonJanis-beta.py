...
class TControl():
	...
	def __init__(self):
		...
		# for the Janis there are 2 temperatures Sample Holder, and Mount		
		self.SensorName = ["Sample", "Mount"]
		self.SensorLocation = [0,1]
		# there is 1 set temp, that of the Sample
		self.SetTemp = np.zeros((1,))
		self.Status = -1

		self.LoopNumber = [1]
		self.ZoneControl = [False]
		self.LoopEnable = [False]
		self.PIDVals = np.empty((3,3))

		# There is 3 heater outputs main heater = VTI, Analog 1 = 4He Pot
		# Analog 2 = Probe
		# so we have the commands to read the heaters
		self.HeaterCommand = ["HTR?"]
		self.HeaterCurrent = np.zeros((3,))
		self.DeltaTemp = np.zeros((4,))
		self.MaxSetTemp = np.array([400.0])
		...
		# Sweep description, temperature denotes Sample temperatures
	...
	# The ls335 often formats replies X,Y,Z -> return selection of values
