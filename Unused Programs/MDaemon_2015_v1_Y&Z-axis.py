...
class MControl():
    ...
    def __init__(self):
        ...
        self.Visa = VisaSubs.InitializeSerial("ASRL5::INSTR")
        # Open the socket
    ...
    def MagnetReadNumeric(self, Command, Axis):
        # Form the query string (Now only for GRPY) Changed to GRPY by Junye
		AxisCommand = "".join(("GRP", Axis))
    ...
    def MagnetReadHeater(self):
        Reply = self.Visa.ask("READ:DEV:GRPY:PSU:SIG:SWHT")
        Answer = string.rsplit(Reply,":",1)[1]
        if Answer == "ON":
            Valid = 1
            self.Heater = True
	...
	self.MagnetSetAction("HOLD")
        ...
        if State:
            Reply = self.Visa.ask("SET:DEV:GRPY:PSU:SIG:SWHT:ON")
        else:
            Reply = self.Visa.ask("SET:DEV:GRPY:PSU:SIG:SWHT:OFF")
    ...
    def MagnetReadAction(self):
        ...
        Reply = self.Visa.ask("READ:DEV:GRPY:PSU:ACTN")
        Answer = string.rsplit(Reply,":",1)[1]
        return Answer
    ...
    def MagnetSetAction(self, Command):
        ...
        Reply = self.Visa.ask("".join(("SET:DEV:GRPY:PSU:ACTN:",Command)))	
