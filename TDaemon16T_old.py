...
class TControl():
	...
	def __init__(self):
		...
		self.PotVisa = VisaSubs.InitializeGPIB(6,0, term_chars = "\\n")
		...
		# set temperature e.g. 0.005 K
		self.ErrorTemp = 0.005
		# The acceptable stability
		self.ErrorDeltaTemp = 0.002
		self.DeltaIndex=1
		self.DeltaArray=[False]*60
	...
	def UpdateAtSet(self):
		...
		Stable = False
		# 1 = Sweep
		ErrorFactor = abs(self.Temperature[1] - self.SetTemp[1])
		DeltaTempFactor = abs(self.DeltaTemp[1])
		if ErrorFactor < self.ErrorTemp:
			Set = True
		if DeltaTempFactor < self.ErrorDeltaTemp:
			Stable = True
		
		self.AtSetTemp = Set and Stable
		
		# Improve the stability before sending AtSet. Wait for 60 stable readouts before sending message.
		self.DeltaIndex=self.DeltaIndex+1
		position=divmod(self.DeltaIndex,60)
		self.DeltaArray[position[1]]=self.AtSetTemp
		#print(self.DeltaArray)
		
		self.AtSet = all(self.DeltaArray)
		return
